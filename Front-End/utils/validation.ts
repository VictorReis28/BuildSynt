...
export const validatePassword = (password: string): ValidationResult => {
  ...
  if (!password) {
    ...
    if (!ValidationPatterns.password.test(password)) {
      errors.push('Senha deve conter pelo menos: 1 letra minúscula, 1 maiúscula, 1 número e 1 caractere especial');
    }
  }
...
export const validateGitHubUrl = (url: string): ValidationResult => {
  ...
  if (!url) {
    ...
    errors.push('URL deve ser um repositório GitHub válido (https://github.com/usuario/repositorio)');
  }
